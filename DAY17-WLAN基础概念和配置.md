...
- **无线 ≠ Wi-Fi**：无线家族还包括蓝牙（短距传文件）、`ZigBee`（物联网低功耗）、红外（遥控器），但`Wi-Fi`是`WLAN`最主流的实现方式。
- **Wi-Fi ≈ 802.11**：在绝大多数实际场景中，`WiFi`等同于`802.11`。

  - `IEEE 802.11`是现今无线局域网通用的标准。由国际电机电子工程学会（`IEEE`）定义的无线网络通信的标准。

    `Wi-Fi`联盟成立于`1999`年，当时名称叫做`Wireless Ethernet Compatibility Alliance(WECA)`。在`2002.10`改名为`Wi-Fi Alliance`。这是一种**创建于IEEE 802.11标准上的无线局域网技术**。
...
### 3.1 CAPWAP协议（无线接入点控制和配置协议）

- **CAPWAP(`Control And Provisioning of Wireless Access Points Protocol`无线接入点控制和配置协议）** ：`AC`和`AP`之间用来建立安全隧道、下发配置、转发数据的专用协议，**基于UDP传输**。
- **两个端口分工明确**：
